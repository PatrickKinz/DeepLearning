...
def create_images(seg,archive_name,multiples,noise,step_size=10):
    ...
    Seg_array    = np.zeros((n*multiples,patch_size,patch_size,1),dtype=np.uint8)
        ...
        for x in range(0,nda_seg_slice.shape[0]-patch_size,step_size):
            for y in range(0,nda_seg_slice.shape[1]-patch_size,step_size):
                ...
                for j in range(multiples):
                    ...
                    #b[3] = stays the same, just converted differently
                    R2[3] = (1-0.5) * b[3] + 0.5 #smaller for CSF than rest
                    c[3] = 0
                    d[3] = 0
                    e[3] = 0
                    Y[3] = 0
                    nu[3] = 0
                    chi_nb[3] = 0
                    """ Segmentation Tissue, Air, CSF """
                    # for Sparse Categorical Loss function encode as integer values Tissue=0, Air=1, CSF=2
                    # add an array that has for each voxel either 0,1 or 2 in the next step
                    ...
                    for xx in range(patch_size):
                        for yy in range(patch_size):
                            ...
                            if type == 0: #air
                                Seg_array[index[count],xx,yy,0] = 1
                            elif type == 3: #CSF
                                Seg_array[index[count],xx,yy,0] = 2
                            #else is tissue which is automatically initialised as 0
                    if noise:
    ...
    np.savez(archive_name,qBOLD=qBOLD_array,QSM=QSM_array,Seg=Seg_array,S0=S0_array,R2=R2_array,Y=Y_array,nu=nu_array,chi_nb=chi_nb_array)

#%%
#Create train and test data separately by adjusting multiples, for example 9 for train and 1 for test
create_images(seg,"../Brain_Phantom/Patches_with_air_big_GESSE/15GB_1Pnoise_train_val",multiples=4,noise=True)
create_images(seg,"../Brain_Phantom/Patches_with_air_big_GESSE/15GB_1Pnoise_test",multiples=1,noise=True,step_size=15)

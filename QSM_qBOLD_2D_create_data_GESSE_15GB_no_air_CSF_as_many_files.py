        ...
        for x in range(0,nda_seg_slice.shape[0]-patch_size,step_size):
            for y in range(0,nda_seg_slice.shape[1]-patch_size,step_size):
                ...
                for j in range(multiples):
                    ...
                    qBOLD_array  = np.zeros((patch_size,patch_size,len(t),1),dtype=np.float32)
                    QSM_array    = np.zeros((patch_size,patch_size,1,1),dtype=np.float32)
                    ...
                    for xx in range(patch_size):
                        for yy in range(patch_size):
                            ...
                            qBOLD_array[xx,yy,:,0]  = qBOLD[type,:]
                            QSM_array[xx,yy,:,0]    = QSM[type]

                    if noise:
                        qBOLD_array = qBOLD_array + rng.normal(loc=0,scale=1./100,size=qBOLD_array.shape)
                        QSM_array   = QSM_array   + rng.normal(loc=0,scale=0.1/100,size=QSM_array.shape)
...
create_images(seg,directory,multiples=1,noise=True)
#%%
directory = "../Brain_Phantom/Patches_no_air_big_GESSE_1pNoise_single_files_test/"

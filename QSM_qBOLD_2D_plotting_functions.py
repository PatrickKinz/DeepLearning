...
def check_Params_transformed_hist(Params_test,p,filename):
    ...
    ax = axes.ravel()
    ax[0].hist2d(x=Params_test[0][:,:,:].ravel(),y=np.squeeze(p[0][:,:,:,:]).ravel(),bins=30,range=((0,1),(0,1)))
    ax[0].title.set_text('S0')
    ax[1].hist2d(x=Params_test[1][:,:,:].ravel(),y=np.squeeze(p[1][:,:,:,:]).ravel(),bins=30,range=((0,30),(0,30)))
    ax[1].title.set_text('R2')
    ax[2].hist2d(x=Params_test[2][:,:,:].ravel(),y=np.squeeze(p[2][:,:,:,:]).ravel(),bins=30,range=((0,1),(0,1)))
    ax[2].title.set_text('Y')
    ax[3].hist2d(x=Params_test[3][:,:,:].ravel(),y=np.squeeze(p[3][:,:,:,:]).ravel(),bins=30,range=((0,.1),(0,.1)))
    ax[3].title.set_text('nu')
    ax[4].hist2d(x=Params_test[4][:,:,:].ravel(),y=np.squeeze(p[4][:,:,:,:]).ravel(),bins=30,range=((-.1,.1),(-.1,.1)))
    ax[4].title.set_text('chi_nb')
    ax[5].remove()
    plt.show()

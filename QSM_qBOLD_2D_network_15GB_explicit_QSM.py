...
model_params = keras.Model(inputs=[input_qBOLD,input_QSM],outputs=[conv_S0,conv_R2,conv_Y,conv_chinb],name="Params_model")
...
model_params_explicit_QSM = keras.Model(inputs=[input_qBOLD,input_QSM],outputs=[conv_S0,conv_R2,limit_Y,limit_nu,limit_chinb],name="Params_model_explicit_QSM")
model_params_explicit_QSM.summary()
keras.utils.plot_model(model_params_explicit_QSM, show_shapes=True)
...
model_params_explicit_QSM.compile(
    loss=losses,
    loss_weights=lossWeights,
    optimizer=opt,
    #metrics=[tf.keras.metrics.MeanAbsolutePercentageError()],
...
training_list = [S0_train,R2_train,Y_train,chi_nb_train]
...
my_callbacks = [
    #tf.keras.callbacks.EarlyStopping(patience=3),
    tf.keras.callbacks.ModelCheckpoint(filepath='models\model_params_explicit_limited_QSM.{epoch:02d}-{val_loss:.4f}.h5'),
    #tf.keras.callbacks.TensorBoard(log_dir='./logs/2021_07_15-1330')
]

training_list = [S0_train,R2_train,Y_train,nu_train,chi_nb_train]

history_params_explicit_QSM = model_params_explicit_QSM.fit([qBOLD_training,QSM_training], training_list , batch_size=100, epochs=20, validation_split=0.1/0.9, callbacks=my_callbacks)
...
test_scores = model_params_explicit_QSM.evaluate([qBOLD_test,QSM_test], test_list, verbose=2)
...
#model_params.summary()
p = model_params_explicit_QSM.predict([qBOLD_test,QSM_test])
p[0].shape
